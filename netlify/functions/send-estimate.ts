type RepairOrderRow = {
  ...
  line_items: Array<{ item_type: string; description: string; quantity: number; unit_price: number; taxable: boolean; sort_order: number; service_group_id: string | null; service_group_title: string | null; technician_story: string | null }>;
};
...
export default async (request: Request) => {
  ...
  const roResponse = await fetch(
    `${supabaseUrl}/rest/v1/repair_orders?select=*,customers(name,email),vehicles(year,make,model,vin),line_items(item_type,description,quantity,unit_price,taxable,sort_order,service_group_id,service_group_title,technician_story)&id=eq.${encodeURIComponent(body.repairOrderId)}&owner_id=eq.${user.id}`,
